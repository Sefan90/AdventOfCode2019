...
def Part1():
	...
	while True:
			...
			for row in splitedlist:
				...
				if element[1] == row[len(row)-1][0][1]:
					if int(element[0]) <= int(row[len(row)-1][0][0]):
						...
						#print(element[0])
						#print(row[len(row)-1][0][0])
						#print(element[0] <= row[len(row)-1][0][0])
							#tempList.append(item[0])
					else:
						#print(math.ceil(int(element[0])/int(row[len(row)-1][0][0])))
						#print(int(element[0])/int(row[len(row)-1][0][0]))
		...
		for i in range(len(tempList)):
			temp2List.append(tempList[i])
			for x in range(len(temp2List)-1):
				if tempList[i][1] == temp2List[x][1]:
					temp2List.pop()
					temp2List[x][0] = str(int(temp2List[x][0]) + int(tempList[i][0]))
					print(temp2List)
					print(temp2List[x][0])
					print(str(int(temp2List[x][0]) + int(tempList[i][0])))
					break

		fuelList = temp2List
		#print(fuelList)
		print("T1: "+ str(tempList))
		print("T2: "+ str(temp2List))
