...
def IntcodeComputer(amp):
	...
	while i < len(input):
		if (modeChecker(input,relative,i) == 1):
			...
			i += 2
			return [input,i,[output],relative]
...
def Part1():
	...
	inputCommand = [inputList,0,startInput,0]
	while True:
		...
		inputCommand[3] = rotation[3]
		if lastXY in paintList.keys():
...
def Part2():
	...
	for x in range(-10,10):
		...
		for y in range(-10,50):
			if  str(x)+","+str(y) in paintList.keys() and paintList[str(x)+","+str(y)] == 1:
				printList.append("#")
			else:
				printList.append(" ")
		print(printList)
