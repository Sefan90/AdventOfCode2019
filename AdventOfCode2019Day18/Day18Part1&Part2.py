...
def IntcodeComputer(input,next_i,startInput):
    ...
    while i < len(input):
        if (modeChecker(input,relative,i) == 1):
            ...
            modeChecker(input,relative,i,1,startInput)
            startInputCounter += 1
            i += 2
...
def Part1():
    result = [["." for x in range(50)] for y in range(50)]
    bigList = [0] * 10000
    ...
    sendList = [bigList,0,startInput]
    while True: #Måste köras för varje punkt i 50x50?
        sendList = IntcodeComputer(sendList[0],sendList[1],sendList[2])
        result[sendList[2]//50][sendList[2]%50] = '#'
        print(sendList[2]%50)
        if sendList[0] == None:
            break
    for i in range(50):
